...
namespace Yezz.Core
{
    public class ConnectionHandling
    {
        ...
        private void manager_connectionEvent(ConnectionInformation connection)
        {
            ...
            YezzEnvironment.GetGame().GetClientManager().CreateAndStartClient(Convert.ToInt32(connection.getConnectionID()), connection);
        }
        ...
        private void CloseConnection(ConnectionInformation Connection)
        {
            ...
            {
                Connection.Dispose();
                YezzEnvironment.GetGame().GetClientManager().DisposeConnection(Convert.ToInt32( Connection.getConnectionID()));
            }
            catch (Exception e)
            {
