...
using Yezz.HabboHotel.Catalog.Pets;
using Yezz.HabboHotel.Catalog.Vouchers;
using Yezz.HabboHotel.Catalog.Marketplace;
using Yezz.HabboHotel.Catalog.Clothing;

using Yezz.Database.Interfaces;
using Yezz.HabboHotel.Catalog.PredesignedRooms;

namespace Yezz.HabboHotel.Catalog
{
    public class CatalogManager
    {
        private static readonly ILog log = LogManager.GetLogger("Yezz.HabboHotel.Catalog.CatalogManager");
                ...
                if (CatalogueItems != null)
                {
                    foreach (DataRow Row in CatalogueItems.Rows)
                    {
                        ...
                        this._items[PageId].Add(Convert.ToInt32(Row["id"]), new CatalogItem(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["item_id"]),
                            ...
                            Convert.ToInt32(Row["amount"]), Convert.ToInt32(Row["limited_sells"]), Convert.ToInt32(Row["limited_stack"]), YezzEnvironment.EnumToBool(Row["offer_active"].ToString()),
