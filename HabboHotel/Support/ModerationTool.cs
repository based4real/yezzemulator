...
//                {
//                    dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'0','','" + YezzEnvironment.GetUnixTimestamp() + "')");
...
//                Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, 0, "", YezzEnvironment.GetUnixTimestamp(), Messages);
...
//            RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Session.GetHabbo().CurrentRoomId);

//            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
//            {
//                dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'" + Data.Id + "',@name,'" + YezzEnvironment.GetUnixTimestamp() + "')");
//                dbClient.AddParameter("message", Message);
...
//            Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, Data.Id, Data.Name, YezzEnvironment.GetUnixTimestamp(), Messages);
//            Tickets.Add(Ticket);
//            SendTicketToModerators(Ticket);
//        }
...
//            GameClient Client = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(Ticket.SenderId);
...
//        {
//            YezzEnvironment.GetGame().GetClientManager().SendMessage(new ModeratorSupportTicketComposer(Ticket), "mod_tool");
//            YezzEnvironment.GetGame().GetClientManager().StaffAlert(RoomNotificationComposer.SendBubble("tickets", "Hay una nueva petición de ayuda, revísala cuanto antes.", ""));
