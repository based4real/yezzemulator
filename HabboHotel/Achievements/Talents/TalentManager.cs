...
namespace Yezz.HabboHotel.Achievements
{
    ...
    public class TalentManager
    {
        ...
        public void CompleteUserTalent(GameClient session, Talent talent)
        {
            ...
            {
                ItemData item = null;
                if (!YezzEnvironment.GetGame().GetItemManager().GetItem(talent.PrizeBaseItem, out item))
                    return;
            ...
            using (IQueryAdapter queryReactor = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
            ...
            if (talent.Type == "citizenship")
            {
                ...
                {
                    case 3:
                        YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(session, "ACH_Citizenship", 1);
                        break;
                    case 4:
