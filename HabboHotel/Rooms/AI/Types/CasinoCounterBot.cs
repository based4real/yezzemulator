...
namespace Yezz.HabboHotel.Rooms.AI.Types
{
    class CasinoCounter : BotAI
    {
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            GameClient Target = YezzEnvironment.GetGame().GetClientManager().GetClientByUsername(Message.Split(' ')[4]);
            if (Target == null)
            {
            ...
            int itemstotal = 0;
            using (IQueryAdapter query = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
