...
namespace Yezz.HabboHotel.Rooms.AI.Types
{
    class BartenderBot : BotAI
    {
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            BotResponse Response = YezzEnvironment.GetGame().GetBotManager().GetResponse(GetBotData().AiType, Message);
            if (Response == null)
                return;
        ...
        public override void OnTimerTick()
        {
            ...
            if (SpeechTimer <= 0)
            {
                if (GetBotData().RandomSpeech.Count > 0)
                {
                    ...
                    string word;
                    string String = YezzEnvironment.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(Speech.Message, out word) ? "Spam" : Speech.Message;
