    ...
    class RouletteBotHonor : BotAI
    {
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            if (User.GetClient().GetHabbo().Duckets >= 2 && BetsOpen == true)
            {
                ...
                User.GetClient().SendMessage(RoomNotificationComposer.SendBubble("eventoxx", User.GetClient().GetHabbo().Username + ", acabas de apostar " + IntMultiplier + "  " + YezzEnvironment.GetDBConfig().DBData["seasonal.currency.name"] + " al " + IntBet + ".\n\n Buena suerte y apuesta con cabeza.", "catalog/open/habbiween"));

                GetRoomUser().Chat("Apuesta de " + IntMultiplier + "  " + YezzEnvironment.GetDBConfig().DBData["seasonal.currency.name"] + " realizada al " + IntBet + " por " + User.GetClient().GetHabbo().Username + ".", false, 33);
        ...
        public override void OnTimerTick()
        {
            ...
            if (GameLength <= 0)
            {
                ...
                foreach (KeyValuePair<int, CasinoDataLocura> pair in Data1)
                {
                    ...
                    if (Data2.bet == num)
                    {
                        GameClient client = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(Data2.userId);
