...
namespace Yezz.HabboHotel.Rooms.Chat.Commands.User.Fun
{
    class MimicCommand : IChatCommand
    {
        ...
        public void Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            if (Params.Length == 1)
            {
                Session.SendWhisper("Introduce el nombre del usuari@ al cual le quieres copiar la ropa.", 34);
                return;
            }
            ...
            {
                Session.SendWhisper("Ocurrió un error, escribe correctamente el nombre o tal vez el usuario no se encuentre online.", 34);
                return;
            }

            if (!TargetClient.GetHabbo().AllowMimic)
            {
                Session.SendWhisper(Params[1] + " tiene desactivada la opción de que le copien el look.", 34);
                return;
            }
