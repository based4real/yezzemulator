                ...
                else
                {
                    ...
                    foreach (Item Chair in Items.ToList())
                    {
                        if (Chair.GetBaseItem().InteractionType == InteractionType.idol_chair)
                        {
                            ...
                            if (!HasUsers)
                            {
                                Session.SendWhisper("No hay ningún juez en la silla de juzgado.", 34);
                                return;
                            }
                            YezzEnvironment.GetGame().GetClientManager().QuizzAlert(new QuickPollMessageComposer(question), Chair, Room);
