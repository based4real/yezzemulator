...
namespace Yezz.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            mStaticModel = YezzEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            if (mStaticModel == null)
            {
                YezzEnvironment.GetGame().GetRoomManager().LoadModel(room.ModelName);
                mStaticModel = YezzEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            }
        ...
        public Point getRandomWalkableSquare()
        {
            ...
            int RandomNumber = YezzEnvironment.GetRandomNumber(0, walkableSquares.Count);
            int i = 0;
        ...
        public bool IsValidWalk(RoomUser User, Vector2D From, Vector2D To, bool Override)
        {
            ...
            if (Items.Count > 0)
            {
                ...
                if (HasGroupGate)
                {
                    ...
                    {
                        Group Group = null;
                        if (!YezzEnvironment.GetGame().GetGroupManager().TryGetGroup(I.GroupId, out Group))
                            return false;
            ...
            if (mCoordinatedItems.ContainsKey(coord))
            {
                ...
                foreach (Item item in itemsFromSquare)
                {
                    if (item.GetBaseItem().InteractionType == InteractionType.RENTABLE_SPACE)
                        ...
                        if (YezzEnvironment.GetGame().GetRentableSpaceManager().GetRentableSpaceItem(item.Id, out _rentableSpace))
                        {
                            ...
                            if (_rentableSpace.OwnerId == Session.GetHabbo().Id && YezzEnvironment.GetUnixTimestamp() < _rentableSpace.ExpireStamp)
                            {
