    ...
    public class RoomUserManager
    {
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.runFastQuery("UPDATE user_roomvisits SET exit_timestamp = '" + YezzEnvironment.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
        ...
        public void OnCycle()
        {
            ...
            try
            {
                ...
                {
                    GameClient client = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (YezzEnvironment.GetUnixTimestamp() > YezzEnvironment.GetUnixTimestamp() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
                                    ...
                                    if (!YezzEnvironment.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room))
                                        return;
                        ...
                        case InteractionType.PINATA:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 158)
                                    {
                                        ...
                                        if (givenHits == 1) YezzEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);

                                        #region Achievements
                                        YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataWhacker", 1);
                                        YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataBreaker", 1);
                                        #endregion
                        ...
                        case InteractionType.PLANT_SEED:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 192)
                                    {
                                        ...
                                        if (givenHits > 5) YezzEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);
                                        {
                                            givenHits = 0;
                                            Item.ExtraData = givenHits.ToString();
                                            Item.UpdateState();
