...
using Yezz.HabboHotel.Items.Data.Toner;
using Yezz.HabboHotel.Rooms.Games.Freeze;
    ...
    public class Room : RoomData
    {
        ...
        public void AddBan(int pId, long Time)
        {
            if (!Bans.ContainsKey(Convert.ToInt32(pId)))
                Bans.Add(pId, YezzEnvironment.GetUnixTimestamp() + Time);

            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("REPLACE INTO `room_bans` VALUES (" + pId + ", " + Id + ", " + (YezzEnvironment.GetUnixTimestamp() + Time) + ")");
            }
        }
        ...
        public bool HasBanExpired(int pId)
        {
            ...
            if (Bans[pId] < YezzEnvironment.GetUnixTimestamp())
                return true;
        ...
        public void LoadPromotions()
        {
            ...
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                {
                    if (Convert.ToDouble(GetPromotion["timestamp_expire"]) > YezzEnvironment.GetUnixTimestamp())
        ...
        {
            using (IQueryAdapter Adapter = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (ForSale.Group != null)
                {
                    ...
                    if (GroupID > 0)
                    {
                        ...
                        foreach (int UserID in ForSale.Group.GetAllMembers)
                        {
                            ...
                            GameClient UserClient = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(UserID);
                            if (UserClient == null)
                                continue;
                    ...
                    YezzEnvironment.GetGame().GetGroupManager().DeleteGroup(ForSale.Group.Id);
        ...
        public bool OnUserShoot(RoomUser User, Item Ball)
        {
            ...
            foreach (Item item in this.GetRoomItemHandler().GetFurniObjects(Ball.GetX, Ball.GetY).ToList())
            {
                if (item.GetBaseItem().ItemName.StartsWith("fball_goal_"))
                {
                    ...
                    User.UnIdle();
                    User.DanceId = 0;
                    YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FootballGoalScored", 1);
        ...
        public void ProcessRoom()
        {
            ...
            try
            {
                ...
                {
                    YezzEnvironment.GetGame().GetRoomManager().UnloadRoom(this);
                    return;
                }
        ...
        private void OnRoomCrash(Exception e)
        {
            ...
            isCrashed = true;
            YezzEnvironment.GetGame().GetRoomManager().UnloadRoom(this, true);
        }
        ...
        public bool CheckMute(GameClient Session)
        {
            ...
            {
                if (MutedUsers[Session.GetHabbo().Id] < YezzEnvironment.GetUnixTimestamp())
                {
        ...
        public void AddChatlog(int Id, string Message)
        {
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("time", YezzEnvironment.GetUnixTimestamp());
                dbClient.RunQuery();
            }
        }
