...
namespace Yezz.HabboHotel.Navigator
{
        ...
        public static void Search(ServerPacket Message, SearchResultList SearchResult, string SearchData, GameClient Session, int FetchLimit)
        {
            ...
            switch (SearchResult.CategoryType)
            {
                ...
                case NavigatorCategoryType.QUERY:
                    {
                        ...
                        if (SearchData.ToLower().StartsWith("owner:"))
                        {
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (GetRooms != null)
                                {
                                    ...
                                    {
                                        RoomData RoomData = YezzEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                        ...
                        else if (SearchData.ToLower().StartsWith("tag:"))
                        {
                            ...
                            ICollection<RoomData> TagMatches = YezzEnvironment.GetGame().GetRoomManager().SearchTaggedRooms(SearchData);
                        ...
                        else if (SearchData.ToLower().StartsWith("group:"))
                        {
                            ...
                            ICollection<RoomData> GroupRooms = YezzEnvironment.GetGame().GetRoomManager().SearchGroupRooms(SearchData);
                            ...
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (Table != null)
                                {
                                    foreach (DataRow Row in Table.Rows)
                                    {
                                        ...
                                        RoomData RData = YezzEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                ...
                case NavigatorCategoryType.FEATURED:
                    ...
                    ICollection<FeaturedRoom> Featured = YezzEnvironment.GetGame().GetNavigator().GetFeaturedRooms();
                    foreach (FeaturedRoom FeaturedItem in Featured.ToList())
                    {
                        if (FeaturedItem == null)
                            continue;

                        RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(FeaturedItem.roomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.PUBLICAS:
                    ...
                    ICollection<FeaturedRoom2> Featured2 = YezzEnvironment.GetGame().GetNavigator().GetFeaturedRooms2();
                    foreach (FeaturedRoom2 FeaturedItem2 in Featured2.ToList())
                    {
                        if (FeaturedItem2 == null)
                            continue;

                        RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(FeaturedItem2.roomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.STAFF_PICKS:
                    {
                        ...
                        ICollection<StaffPick> picks = YezzEnvironment.GetGame().GetNavigator().GetStaffPicks();
                        foreach (StaffPick pick in picks.ToList())
                        {
                            if (pick == null)
                                continue;

                            RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(pick.roomId);
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.Lote_Salas:
                    {
                        ...
                        ICollection<LoteSalas> lotesalas = YezzEnvironment.GetGame().GetNavigator().GetLoteRooms();
                        foreach (LoteSalas lote in lotesalas.ToList())
                        {
                            if (lote == null)
                                continue;

                            RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(lote.roomId);
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.POPULAR:
                    {
                        List<RoomData> PopularRooms = YezzEnvironment.GetGame().GetRoomManager().GetPopularRooms(-1, FetchLimit);
                ...
                case NavigatorCategoryType.RECOMMENDED:
                    {
                        List<RoomData> RecommendedRooms = YezzEnvironment.GetGame().GetRoomManager().GetRecommendedRooms(FetchLimit);
                ...
                case NavigatorCategoryType.CATEGORY:
                    {
                        List<RoomData> GetRoomsByCategory = YezzEnvironment.GetGame().GetRoomManager().GetRoomsByCategory(SearchResult.Id, FetchLimit);
                ...
                case NavigatorCategoryType.MY_FAVORITES:
                    ...
                    {
                        RoomData Room = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Id);
                        if (Room == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_GROUPS:
                    ...
                    foreach (Group Group in YezzEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id).ToList())
                    {
                        if (Group == null)
                            continue;

                        RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_HISTORY:
                    ...
                    using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        {
                            RoomData Data = YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Convert.ToInt32(Row["room_id"]));
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.TOP_PROMOTIONS:
                    {
                        List<RoomData> GetPopularPromotions = YezzEnvironment.GetGame().GetRoomManager().GetOnGoingRoomPromotions(16, FetchLimit);
                ...
                case NavigatorCategoryType.PROMOTION_CATEGORY:
                    {
                        List<RoomData> GetPromotedRooms = YezzEnvironment.GetGame().GetRoomManager().GetPromotedRooms(SearchResult.Id, FetchLimit);
