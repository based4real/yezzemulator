...
namespace Yezz.HabboHotel.Items.Wired.Boxes.Effects
{
    class GiveRewardBox : IWiredItem
    {
        ...
        public bool Execute(params object[] Params)
        {
            ...
            Habbo Owner = YezzEnvironment.GetHabboById(Item.UserID);
            ...
            if (!unique)
            {
                ...
                var random = YezzEnvironment.GetRandomNumber(0, 100);
                    ...
                    if (code.StartsWith("diamonds:"))
                    {
                        foreach (var reward in code.Split('-'))
                        {
                            ...
                            if (diamonds > 100)
                            {
                                ...
                                YezzEnvironment.GetGame().GetClientManager().StaffAlert(RoomNotificationComposer.SendBubble("advice", "" + Player.GetClient().GetHabbo().Username + " ha usado un wired que no sigue las normas del hotel.\n             Click aquí.", "event:navigator/goto/" + Player.GetClient().GetHabbo().CurrentRoomId));
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    if (!YezzEnvironment.GetGame().GetItemManager().GetItem(int.Parse(code), out ItemData))
                    {
