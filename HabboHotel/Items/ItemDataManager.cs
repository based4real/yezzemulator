...
namespace Yezz.HabboHotel.Items
{
    public class ItemDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Yezz.HabboHotel.Items.ItemDataManager");
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = YezzEnvironment.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = YezzEnvironment.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = YezzEnvironment.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = YezzEnvironment.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = YezzEnvironment.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = YezzEnvironment.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = YezzEnvironment.EnumToBool(Row["is_rare"].ToString());
                            int ClothingId = Convert.ToInt32(Row["clothing_id"]);
                            bool ExtraRot = YezzEnvironment.EnumToBool(Row["extra_rot"].ToString());
