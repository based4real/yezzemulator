...
namespace Yezz.HabboHotel.Items.RentableSpaces
{
    public class RentableSpaceManager
    {
        private static readonly ILog log = LogManager.GetLogger("Yezz.HabboHotel.Items.RentableSpaces");
                ...
                if (table != null)
                {
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            if (ownerid > 0)
                            {
                                Habbo owner = YezzEnvironment.GetHabboById(ownerid);
                                if (owner != null)
        ...
        public bool ConfirmBuy(GameClient Session, RentableSpaceItem RentableSpace, int ExpireSeconds)
        {
            ...
            RentableSpace.ExpireStamp = (int)YezzEnvironment.GetUnixTimestamp() + ExpireSeconds;
            ...
            using (IQueryAdapter con = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                con.AddParameter("expire", YezzEnvironment.GetUnixTimestamp() + 604800);
                con.RunQuery();
            }
            return true;
