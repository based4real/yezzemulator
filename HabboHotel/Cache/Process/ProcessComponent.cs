...
namespace Yezz.HabboHotel.Cache.Process
{
    sealed class ProcessComponent
    {
        private static readonly ILog log = LogManager.GetLogger("Yezz.HabboHotel.Cache.Process.ProcessComponent");
        ...
        public void Run(object State)
        {
            try
            {
                ...
                // BEGIN CODE
                List<UserCache> CacheList = YezzEnvironment.GetGame().GetCacheManager().GetUserCache().ToList();
                if (CacheList.Count > 0)
                {
                    foreach (UserCache Cache in CacheList)
                    {
                        try
                        {
                            ...
                            if (Cache.isExpired())
                                YezzEnvironment.GetGame().GetCacheManager().TryRemoveUser(Cache.Id, out Temp);
                ...
                CacheList = null;
                    ...
                    List<Habbo> CachedUsers = YezzEnvironment.GetUsersCached().ToList();
                        ...
                        foreach (Habbo Data in CachedUsers)
                        {
                            try
                            {
                                ...
                                if (Data.CacheExpired())
                                    YezzEnvironment.RemoveFromCache(Data.Id, out Temp);
