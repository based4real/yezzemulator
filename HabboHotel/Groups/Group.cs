...
namespace Yezz.HabboHotel.Groups
{
    public class Group
    {
        ...
        public void CreateGroupChat(Group group)
        {
            ...
            group.HasChat = true;
            List<GameClient> GroupMembers = (from Client in YezzEnvironment.GetGame().GetClientManager().GetClients.ToList() where Client != null && Client.GetHabbo() != null && this.IsMember(Client.GetHabbo().Id) select Client).ToList();
        ...
        //    foreach (var client in YezzEnvironment.GetGame().GetClientManager().GetClientsById(GetAllMembers))
        //    {
