...
using Yezz.HabboHotel.Users.Process;
using Yezz.Communication.Packets.Outgoing.Inventory.Purse;

using Yezz.HabboHotel.Users.Navigator.SavedSearches;
...
namespace Yezz.HabboHotel.Users
{
    public class Habbo
    {
        private static readonly ILog log = LogManager.GetLogger("Yezz.HabboHotel.Users");
         ...
         byte guidelevel, byte publicistalevel, byte builder, byte croupier, string citizenShip, bool Nux, bool NuxRoom, byte TargetedBuy, string NameColor, string Chatcolour, string Tag, string TagColor, int BubbleId, byte NameChange, string PinClient, int RespetosForbi)
        {
            ...
            this._rank = Rank;
            this._motto = Motto;
            this._look = YezzEnvironment.GetGame().GetAntiMutant().RunLook(Look);
            this._gender = Gender.ToLower();
            this._footballLook = YezzEnvironment.FilterFigure(Look.ToLower());
            ...
            if (this._tradingLockExpiry > 0 && YezzEnvironment.GetUnixTimestamp() > this.TradingLockExpiry)
            {
            ...
            this._sessionStart = YezzEnvironment.GetUnixTimestamp();
            this._messengerSpamCount = 0;
            this._messengerSpamTime = 0;
            this._creditsTickUpdate = YezzStaticGameSettings.UserCreditsUpdateTimer;
            this._diamantesTickUpdate = YezzStaticGameSettings.UserDiamantesUpdateTimer;
            this._hofTickUpdate = YezzStaticGameSettings.halloffame;
            this._bonusTickUpdate = YezzStaticGameSettings.BonusRareUpdateTimer;
            ...
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                try
                {
                    ...
                    if (Convert.ToString(StatRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        else if (YezzEnvironment.GetGame().GetSubscriptionManager().TryGetSubscriptionData(VIPRank, out SubData))
                            DailyRespects = this._respetosforbi;
            ...
            Group G = null;
            if (!YezzEnvironment.GetGame().GetGroupManager().TryGetGroup(this._habboStats.FavouriteGroupId, out G))
        ...
        public Room CurrentRoom
        {
            get
            {
                ...
                Room _room = null;
                if (YezzEnvironment.GetGame().GetRoomManager().TryGetRoom(CurrentRoomId, out _room))
                    return _room;
        ...
        {
            int level = this._Talents.Values.Select(current => YezzEnvironment.GetGame().GetTalentManager().GetTalent(current.TalentId).Level).Concat(new[] { 1 }).Max();
            return level;
        }
        ...
        public void OnDisconnect()
        {
            ...
            if(_onHelperDuty)
            {
                GameClient Session = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(this.Id);
                HelperToolsManager.RemoveHelper(Session);
            }
            
            YezzEnvironment.GetGame().GetClientManager().UnregisterClient(Id, Username);
            ...
            if (!this._habboSaved) // GUARDADO DE USER
            {
                ...
                using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.runFastQuery("UPDATE `users` SET `online` = '0', `last_online` = '" + YezzEnvironment.GetUnixTimestamp() + "', `activity_points` = '" + this.Duckets + "', `credits` = '" + this.Credits + "',  `vip_points` = '" + this.Diamonds + "' ,  `bonus_points` = '" + this._BonusPoints + "', `home_room` = '" + this.HomeRoom + "', `gotw_points` = '" + this.GOTWPoints + "', `puntos_eventos` = '" + this.UserPoints + "', `time_muted` = '" + this.TimeMuted + "',`friend_bar_state` = '" + FriendBarStateUtility.GetInt(this._friendbarState) + "' WHERE id = '" + Id + "' LIMIT 1;UPDATE `user_stats` SET `roomvisits` = '" + this._habboStats.RoomVisits + "', `onlineTime` = '" + (YezzEnvironment.GetUnixTimestamp() - this.SessionStart + this._habboStats.OnlineTime) + "', `respect` = '" + this._habboStats.Respect + "', `respectGiven` = '" + this._habboStats.RespectGiven + "', `giftsGiven` = '" + this._habboStats.GiftsGiven + "', `giftsReceived` = '" + this._habboStats.GiftsReceived + "', `dailyRespectPoints` = '" + this._habboStats.DailyRespectPoints + "', `dailyPetRespectPoints` = '" + this._habboStats.DailyPetRespectPoints + "', `AchievementScore` = '" + this._habboStats.AchievementPoints + "', `quest_id` = '" + this._habboStats.QuestID + "', `quest_progress` = '" + this._habboStats.QuestProgress + "', `groupid` = '" + this._habboStats.FavouriteGroupId + "',`forum_posts` = '" + this._habboStats.ForumPosts + "',`PurchaseUsersConcurrent` = '" + this._habboStats.PurchaseUsersConcurrent + "' WHERE `id` = '" + this.Id + "' LIMIT 1;");
        ...
        public void CheckBonusTimer()
        {
            try
            {
                ...
                if (this._bonusTickUpdate <= 0)
                {
                    ...
                    this.BonusUpdateTick = YezzStaticGameSettings.BonusRareUpdateTimer;
                }
            }
            catch { }
        }
        public void CheckCreditsTimer()
        {
            try
            {
                ...
                {
                    int CreditUpdate = YezzStaticGameSettings.UserCreditsUpdateAmount;
                    int DucketUpdate = YezzStaticGameSettings.UserPixelsUpdateAmount;
                    int VipDucketUpdate = YezzStaticGameSettings.UserVipPixelsUpdateAmount;
                    ...
                    if (this._client.GetHabbo().Rank > 1)
                    {
                        this.GetClient().SendMessage(RoomNotificationComposer.SendBubble("newuser", "Has recibido " + VipDucketUpdate + " duckets por estar conectado 15 minutos.", ""));

                    }
                    else
                    {
                        this.GetClient().SendMessage(RoomNotificationComposer.SendBubble("newuser", "Has recibido créditos y " + DucketUpdate + " duckets  por estar conectado 15 minutos.", ""));
                    }
                    this.CreditsUpdateTick = YezzStaticGameSettings.UserCreditsUpdateTimer;
                }
            }
            catch { }
        }
        ...
        public void CheckDiamondsTimer()
        {
            try
            {
                ...
                {
                    int DiamantesUpdate = YezzStaticGameSettings.UserDiamantesUpdateAmount;
                    ...
                    this.DiamantesUpdateTick = YezzStaticGameSettings.UserDiamantesUpdateTimer;
                }
            }
            catch { }
        }
        ...
        public void CheckhofTimer()
        {
            try
            {
                ...
                if (this._hofTickUpdate <= 0)
                {
                    ...
                    this.HofUpdateTick = YezzStaticGameSettings.halloffame;
                }
            }
            catch { }
        }
        ...
        public GameClient GetClient()
        {
            ...
            return YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(Id);
        }
        ...
        {
            this.LastNameChange = YezzEnvironment.GetUnixTimestamp();
        ...
        public void PrepareRoom(int Id, string Password)
        {
            ...
            {
                Room OldRoom = null;
                if (!YezzEnvironment.GetGame().GetRoomManager().TryGetRoom(this.GetClient().GetHabbo().CurrentRoomId, out OldRoom))
                    return;
            ...
            Room Room = YezzEnvironment.GetGame().GetRoomManager().LoadRoom(Id);
            if (Room == null)
            {
                this.GetClient().SendMessage(new CloseConnectionComposer(this.GetClient()));
                return;
        ...
        {
            if (!YezzEnvironment.GetGame().GetCalendarManager().CampaignEnable())
                return;

            calendarGift = new bool[YezzEnvironment.GetGame().GetCalendarManager().GetTotalDays()];
            ...
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("name", YezzEnvironment.GetGame().GetCalendarManager().GetCampaignName());
        ...
        public bool EnterRoom(Room Room)
        {
            ...
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ('" + this.GetClient().GetHabbo().Id + "','" + this.GetClient().GetHabbo().CurrentRoomId + "','" + YezzEnvironment.GetUnixTimestamp() + "','0','" + DateTime.Now.Hour + "','" + DateTime.Now.Minute + "');");// +
            }
            ...
            if (Room.OwnerId != this.Id)
            {
                ...
                YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(this.GetClient(), "ACH_RoomEntry", 1);            
            }
            return true;
        }
    }
