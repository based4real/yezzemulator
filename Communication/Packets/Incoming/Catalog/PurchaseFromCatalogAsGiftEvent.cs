...
namespace Yezz.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogAsGiftEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (YezzEnvironment.GetDBConfig().DBData["gifts_enabled"] != "1")
            {
            ...
            if (!YezzEnvironment.GetGame().GetItemManager().GetGift(SpriteId, out PresentData) || PresentData.InteractionType != InteractionType.GIFT)
                return;
            ...
            Habbo Habbo = YezzEnvironment.GetHabboByUsername(GiftUser);
            if (Habbo == null)
            {
                    ...
                    case InteractionType.LANDSCAPE:
                        ...
                        try
                        {
                            if (string.IsNullOrEmpty(Data))
                                ...
                                Number = Double.Parse(Data, YezzEnvironment.CultureInfo);
                        }
                        catch
                        {
            ...
            if (GiveItem != null)
            {
                GameClient Receiver = YezzEnvironment.GetGame().GetClientManager().GetClientByUserID(Habbo.Id);
                if (Receiver != null)
                {
                    ...
                    YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_GiftGiver", 1);
                    YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Receiver, "ACH_GiftReceiver", 1);
                    YezzEnvironment.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.GIFT_OTHERS);
