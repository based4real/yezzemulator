...
using Yezz.HabboHotel.Items.Utilities;
...
namespace Yezz.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogEvent : IPacketEvent
    {
        ...
        {
            if (YezzEnvironment.GetDBConfig().DBData["catalogue_enabled"] != "1")
            {
            ...
            if (baseItem != null)
            {
                ...
                if (baseItem.InteractionType == InteractionType.tag)
                {
                    ...
                    {
                        Session.SendMessage(RoomNotificationComposer.SendBubble("advice", "Recuerda, que tu prefijo solo puede ser de 1 a 7 caracteres.", ""));
                        return;
                    }
                    ...
                    {
                        Session.SendMessage(RoomNotificationComposer.SendBubble("advice", "Lo sentimos, no está permitido añadir etiquetas HTML en los prefijos.", ""));
                        return;
                    }
                    ...
                    if (!YezzEnvironment.IsValidAlphaNumeric(ExtraData))
                    {
                    ...
                    string character;
                    ExtraData = YezzEnvironment.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(ExtraData, out character) ? "" : ExtraData;

                    if (string.IsNullOrEmpty(ExtraData))
                    {
                        Session.SendNotification("Lo sentimos, pero la plabara " + character + " no es una palabra permitida dentro de los filtros de " + YezzEnvironment.GetDBConfig().DBData["hotel.name"] + ", por ello no puedes ponerlo");
                        return;
                    }
            ...
            if (Item.IsLimited)
            {
                ...
                YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_LTDPurchased", 1);
            ...
            switch (Item.Data.InteractionType)
            {
                ...
                case InteractionType.GUILD_CHAT:
                    Group thegroup;
                    if (!YezzEnvironment.GetGame().GetGroupManager().TryGetGroup(Convert.ToInt32(ExtraData), out thegroup))
                        return;
                    if (!(YezzEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id).Contains(thegroup)))
                    {
                        return;
                    }
                ...
                case InteractionType.GUILD_FORUM:
                    ...
                    if (!YezzEnvironment.GetGame().GetGroupManager().TryGetGroup(GpId, out Gp))
                    {
                    ...
                    Gf = YezzEnvironment.GetGame().GetGroupForumManager().CreateGroupForum(Gp);
                ...
                case InteractionType.LANDSCAPE:
                    ...
                    try
                    {
                        if (string.IsNullOrEmpty(ExtraData))
                            ...
                            Number = Double.Parse(ExtraData, YezzEnvironment.CultureInfo);
                    }
                    catch (Exception e)
                    {
                        ...
                        default:
                            if (AmountPurchase > 1)
                            {
                                ...
                                if (Items != null)
                                {
                                    ...
                                    Session.SendMessage(RoomNotificationComposer.SendBubble("icon/" + Item.Data.ItemName + "_icon", "Acabas de comprar unos/unas " + Item.Data.PublicName + "", "inventory/open/furni"));
                                }
                            }
                            else
                            {
                                ...
                                if (NewItem != null)
                                {
                                    ...
                                    Session.SendMessage(RoomNotificationComposer.SendBubble("icon/" + Item.Data.ItemName + "_icon", "Acabas de comprar un/una " + Item.Data.PublicName + "", "inventory/open/furni"));
                                }
                            }
                            break;
                ...
                case "p":
                    {
                        ...
                        if (YezzEnvironment.GetGame().GetItemManager().GetItem(320, out PetFood))
                        {
