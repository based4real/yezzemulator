...
namespace Yezz.Communication.Packets.Incoming.QuickPolls
{
    class SubmitPollAnswerMessageEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (questionId == -1)
            {
                ...
                if (room.poolQuestion != string.Empty)
                {
                    ...
                    if (answer.Equals("1"))
                    {
                        ...
                        YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_PollVote", 1);
                    }
                    else
                    {
            ...
            else if (YezzEnvironment.GetGame().GetPollManager().TryGetPollForRoom(room.Id, out poll))
            {
