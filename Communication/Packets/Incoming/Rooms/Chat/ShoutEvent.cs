...
namespace Yezz.Communication.Packets.Incoming.Rooms.Chat
{
    public class ShoutEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            YezzEnvironment.GetGame().GetChatManager().GetLogs().StoreChatlog(new Yezz.HabboHotel.Rooms.Chat.Logs.ChatlogEntry(Session.GetHabbo().Id, Room.Id, Message, UnixTimestamp.GetNow(), Session.GetHabbo(), Room));
                ...
                YezzEnvironment.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(Message, out word))
            {
                ...
                {
                    Session.SendWhisper("¡Has mencionado una palabra no apta para el código de " + YezzEnvironment.GetDBConfig().DBData["hotel.name"] + "! Aviso " + Session.GetHabbo().BannedPhraseCount + "/10");
                ...
                {
                    YezzEnvironment.GetGame().GetModerationManager().BanUser("System", HabboHotel.Moderation.ModerationBanType.USERNAME, Session.GetHabbo().Username, "Baneado por hacer spam con la frase (" + Message + ")", (YezzEnvironment.GetUnixTimestamp() + 78892200));
                    Session.Disconnect();
                    return;
                }
