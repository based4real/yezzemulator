...
namespace Yezz.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Room.GetRoomItemHandler().GetWallAndFloor.Count() > YezzStaticGameSettings.RoomFurnitureLimit)
            {
                Session.SendNotification("no se puede tener mas de " + YezzStaticGameSettings.RoomFurnitureLimit + " furnis en una sala!");
                return;
            }
                ...
                if (Room.GetGameMap().IsRentableSpace(X, Y, Session))
                {
                    ...
                    if (Room.GetRoomItemHandler().SetFloorItem(Session, RoomItem, X, Y, Rotation, true, false, true))
                    {
                        ...
                        if (Session.GetHabbo().Id == Room.OwnerId)
                            YezzEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_RoomDecoFurniCount", 1, false);
