...
namespace Yezz.Communication.Packets.Incoming.Groups
{
    class GetForumsListDataEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                case 2:
                    var Forums = YezzEnvironment.GetGame().GetGroupForumManager().GetForumsByUserId(Session.GetHabbo().Id);
                ...
                case 0:
                    ...
                    using (var adap = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        adap.AddParameter("now", (int)YezzEnvironment.GetUnixTimestamp());
                    ...
                    {
                        GroupForum forum;
                        if (YezzEnvironment.GetGame().GetGroupForumManager().TryGetForum(Convert.ToInt32(Row["id"]), out forum))
                            forums.Add(forum);
                    }
                    break;
