...
namespace Yezz.Communication.Packets.Incoming.Groups
{
    class DeleteGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Group.MemberCount >= YezzStaticGameSettings.GroupMemberDeletionLimit && !Session.GetHabbo().GetPermissions().HasRight("group_delete_limit_override"))
            {
                Session.SendMessage(new RoomNotificationComposer("Sucesso",
                 "El grupo sobre pasa el límite de miembros permitido (" + YezzStaticGameSettings.GroupMemberDeletionLimit + "), contacta con uno de los miembros del equipo administrativo.", "nothing", ""));
                return;
            }

            Room Room = YezzEnvironment.GetGame().GetRoomManager().LoadRoom(Group.RoomId);
            ...
            //Removê-lo do cache.
            YezzEnvironment.GetGame().GetGroupManager().DeleteGroup(Group.Id);
