...
namespace Yezz.Communication.Packets.Outgoing.Groups
{
    class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
            ...
            base.WriteString((YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "No room found.." : YezzEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
            ...
            base.WriteString(YezzEnvironment.GetUsernameById(Group.CreatorId));
