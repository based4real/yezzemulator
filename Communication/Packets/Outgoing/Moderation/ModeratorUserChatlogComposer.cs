...
namespace Yezz.Communication.Packets.Outgoing.Moderation
{
    class ModeratorUserChatlogComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserChatlogMessageComposer)
        {
           ...
           base.WriteString(YezzEnvironment.GetGame().GetClientManager().GetNameById(UserId));
            using (IQueryAdapter dbClient = YezzEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Visits != null)
                {
                    ...
                    foreach (DataRow Visit in Visits.Rows)
                    {
                        ...
                        Room Room = YezzEnvironment.GetGame().GetRoomManager().LoadRoom(Convert.ToInt32(Visit["room_id"]));
                        ...
                        {
                            Visit["exit_timestamp"] = YezzEnvironment.GetUnixTimestamp();
                        }
                        ...
                        if (Chatlogs != null)
                        {
                            ...
                            {
                                UserCache Habbo = YezzEnvironment.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(Log["user_id"]));
